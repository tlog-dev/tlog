...
Almost all allocs are eliminated. Only sugared interface make allocs, there are tools to avoid it.
...
It's powerful logger. Besides usual logging like stdlib `log` there are structured logging, conditional logging, testing logging, `Labels`, `Attributes`, `Callers` info attachment.

It combines three systems at one.
You do things once and use it reading logs, investigating traces or evaluating metrics.
Everything is ready to work on distributed systems with no additional cost.
...
It's going to mimic of and/or integrate with existing systems: prometheus, OpenTelemetry, `net/trace`, Google Trace API, maybe jaeger backend...
